...
class CapsuleLayer(nn.Module):
    ...
    def _forward(self, expanded_x, weights):
        ...
        squashed_caps_consensus = self._squash(caps_consensus)  # (B, M, Cout)

        unweighted_transformed_x = transformed_x / weights  # (B, M, N, Cout)

        return unweighted_transformed_x, squashed_caps_consensus
    ...
    def _dynamic_agreement_routing(self, x):
        ...
        weights = (1 / self.n_out_caps) * torch.ones((x.shape[0], self.n_out_caps, self.n_in_caps))  # (B, M, N)
        ...
        for i in range(self.agreement_iterations):
            ...
            weights = weights + func.softmax(caps_assignment_logits, dim=1)  # (B, M, N)
    ...
    def _squash(self, x):
        ...
        """
            ...
            -------
            torch.tensor
                squash transformation applied onto argument x
        """
    ...
    def forward(self, x):
        """
            ...
            -------
            torch.tensor
                Torch tensor of shape (B, M, Cout), where B is the batch
        """
